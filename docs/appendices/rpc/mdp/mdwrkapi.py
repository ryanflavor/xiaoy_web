...
class MajorDomoWorker(object):
    ...
    def disconnect_broker(self):
        ...
        # 发送断开连接的消息给broker
        self.send_to_broker(MDP.W_DISCONNECT)
        # 从poller中注销该socket
        self.poller.unregister(self.worker)
        # 关闭与broker的连接
        self.worker.close()
        # 设置worker为None，表示当前没有与broker的连接
        self.worker = None
        if self.verbose:
            logging.info("I: disconnected from broker at %s...", self.broker)
        ...
        while True:
            ...
            if items:
                ...
                if command == MDP.W_REQUEST:
                    ...
                    if (msg and not self.designated) or (not msg and self.designated):
                        self.designated = not self.designated
                        self.designate_switch()
                        # print(f"{datetime.datetime.now()} designated: {self.designated}")
